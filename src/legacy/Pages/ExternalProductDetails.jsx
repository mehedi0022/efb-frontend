...
const IMAGE_BASE = (() => {
  const viteBase =
    ...
    import.meta?.env &&
    import.meta.env.VITE_EXTERNAL_IMAGE_BASE;
  if (viteBase) return String(viteBase);
  return process.env.NEXT_PUBLIC_EXTERNAL_IMAGE_BASE || "http://localhost:5000/";
})();
...
const mapRelatedProducts = (items) => {
  ...
  return items.map((item) => {
    ...
    return {
      ...
      oldPrice:
        item?.previous_price ?? info?.previous_price ?? info?.old_price ?? null,
...
const ExternalProductDetails = () => {
  ...
  const slug = useMemo(
    () => normalizeExternalProductSlug(routeSlug),
    [routeSlug],
  );
  ...
  const {
    data: externalShippingResponse,
    isLoading: externalShippingLoading,
    isFetching: externalShippingFetching,
  } = useGetExternalShippingChargesQuery();
  ...
  const selectedVariant = useMemo(() => {
    ...
    if (Number(selectedSize) > 0) {
      return (
        variants.find((item) => Number(item?.id) === Number(selectedSize)) ||
        null
      );
    }
  ...
  const variantPrice = Number(selectedVariant?.price ?? NaN);
  const price = Number.isFinite(variantPrice) ? variantPrice : basePrice;
  const previousPrice = data?.previousPrice ?? data?.previous_price ?? "";
  const sku =
    selectedVariant?.sku ?? data?.sku ?? data?.productSku ?? data?.product_code ?? "";
  const categoryLabel =
    data?.category_name ||
    data?.categoryName ||
    data?.category?.name ||
  ...
  const stockRaw =
    selectedVariant?.stock ?? data?.availableStock ?? data?.available_stock ?? data?.stock;
  const hasStockValue =
  ...
  const panelMeta = useMemo(() => resolvePanelOrderMeta(data || {}), [data]);
  ...
  const buildExternalPayload = () => {
    const sizeObj = sizes.find(
      (item) => String(item.id) === String(selectedSize),
    );
    const colorObj = colors.find(
      (item) => String(item.id) === String(selectedColor),
    );
    const externalProductId =
      data?.id || data?.productId || data?.product_id || sku || slug;
    const selectedVariantId = Number(selectedVariant?.id) > 0 ? Number(selectedVariant.id) : null;
    const panelVariantId = selectedVariantId || panelMeta.panelVariantId;
  ...
  const handleAddToCart = async (redirectToCheckout = false) => {
    try {
      ...
      const message =
        requestError?.data?.errors?.message ||
  ...
  return (
    <ProductDetailView
      ...
      youtubeVideoUrl={
        data?.youtube_video_url || data?.youtube_url || data?.video_url || ""
      }
      ...
      shippingChargesOverride={externalShippingResponse?.data || []}
      shippingLoadingOverride={externalShippingLoading || externalShippingFetching}
    />
  );
};
