...
const resolveImage = (src, base = IMAGE_BASE) => {
    ...
    const normalizedSrc = typeof src === 'object' && src !== null ? (src.image || src.path || '') : src;
...
const Home = () => {
    ...
    const featuredCategories = Array.isArray(featuredResponse?.data) ? featuredResponse.data : [];
    ...
    const topSell = Array.isArray(topSellResponse?.data) ? topSellResponse.data : [];
    const topSellMeta = {
    ...
    const hotDeals = Array.isArray(hotDealsResponse?.data) ? hotDealsResponse.data : [];
    const hotDealsMeta = {
    ...
    const categorySections = Array.isArray(categoryResponse?.data) ? categoryResponse.data : [];
    const categoryMeta = {
        current_page: categoryResponse?.meta?.product_pagination?.current_page || categoryPage,
    ...
    const homeCategories = Array.isArray(homeCategoriesResponse?.data) ? homeCategoriesResponse.data : [];
    ...
    const normalizedBannerLink = bannerLink
        ? (isExternalHttpUrl(bannerLink)
            ? bannerLink
            : (bannerLink.startsWith('/') ? bannerLink : `/${bannerLink}`))
        : '';
    ...
    return (
        <div className="bg-[#f3f4f6]">
                    ...
                    <div className="h-48 sm:h-64 md:h-80 overflow-hidden">
                        {normalizedBannerLink ? (
                            isExternalHttpUrl(normalizedBannerLink) ? (
                                <a href={normalizedBannerLink} target="_blank" rel="noreferrer" className="block h-full w-full">
                                    <img
                                        src={bannerImage}
            ...
            <section className="max-w-6xl mx-auto px-4 mt-10">
                <h3 className="text-lg font-semibold text-center mb-4">Featured Category</h3>
                    ...
                    >
                        {loadingFeatured ? (
                            Array.from({ length: 8 }).map((_, idx) => (
                                <div key={`featured-skeleton-${idx}`} className="min-w-[140px] bg-white rounded-lg shadow-sm p-3 flex flex-col items-center">
                                    <div className="h-12 w-12 rounded-full bg-gray-200 animate-pulse" />
                                    <div className="mt-3 h-3 w-16 rounded bg-gray-200 animate-pulse" />
                                </div>
                            ))
                        ) : featuredCategories.length === 0 ? (
                            <div className="w-full py-6 text-center text-sm text-gray-500">No featured categories found.</div>
                        ) : (
            ...
            {(loadingHomeCategories || homeCategories.length > 0) && (
                <section className="max-w-6xl mx-auto px-4 mt-10">
                    <div className="space-y-8">
                        {(loadingHomeCategories ? Array.from({ length: 2 }) : homeCategories).map((cat, idx) => {
                            ...
                            return (
                                <div key={catSlug || idx}>
                                    <div className="flex items-center justify-between mb-3">
                                        <h3 className="text-lg font-semibold text-gray-800">{loadingHomeCategories ? '' : catName}</h3>
                                        {!loadingHomeCategories && (
                                            <Link to={`/products?category=${catSlug}`} className="text-xs font-semibold text-gray-600 hover:text-black">SEE MORE</Link>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                        {(loadingHomeCategories ? Array.from({ length: 5 }) : catProducts).map((item, productIdx) => (
                                            <div key={item?.id || item?.slug || productIdx}>
                                                {loadingHomeCategories ? <SkeletonCard /> : <StorefrontProductCard item={item} />}
                                            </div>
                                        ))}
                                    </div>
                                </div>
            ...
            <section className="max-w-6xl mx-auto px-4 mt-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {(loadingHotDeals ? Array.from({ length: 12 }) : hotDeals).map((item, idx) => (
                        <div key={item?.id || item?.product_info?.slug || idx}>
                            {loadingHotDeals ? <SkeletonCard /> : <StorefrontProductCard item={item} />}
                        </div>
                    ))}
                </div>
                <Pagination current={hotDealsMeta.page} last={hotDealsMeta.last_page} onPage={handleHotDealsPage} />
            </section>
            ...
            <section className="max-w-6xl mx-auto px-4 mt-12 pb-16">
                ...
                <div className="space-y-8">
                    {(loadingCategories ? Array.from({ length: 3 }) : categorySections).map((category, idx) => {
                        const name = resolveExternalCategoryName(category);
                        const slug = resolveExternalCategorySlug(category);
                        ...
                        return (
                            <div key={slug || idx}>
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <Link to={`/products?category=${slug}`} className="text-xs font-semibold text-gray-600">SEE MORE</Link>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                    {(loadingCategories ? Array.from({ length: 5 }) : products).map((item, productIndex) => (
                                        <div key={item?.id || item?.product_info?.slug || productIndex}>
                                            {loadingCategories ? <SkeletonCard /> : <StorefrontProductCard item={item} />}
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                <div className="flex justify-center mt-6">
                    <div className="flex items-center gap-2">
                        <button
                            type="button"
                            onClick={() => handleCategoryPage(Math.max(1, categoryMeta.current_page - 1))}
                        ...
                        <button
                            type="button"
                            onClick={() => handleCategoryPage(Math.min(categoryMeta.last_page, categoryMeta.current_page + 1))}
