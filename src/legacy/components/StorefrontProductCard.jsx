...
const StorefrontProductCard = ({ item, product }) => {
  ...
  const priceValue =
    source?.price ?? info?.price ?? info?.selling_price ?? info?.new_price ?? 0;
  const previousPriceValue =
    source?.previous_price ?? info?.previous_price ?? info?.old_price ?? 0;
  ...
  const discountPercent =
    previousPrice > 0 && discountAmount > 0
  ...
  const stockValue =
    stockValueRaw === null ||
    stockValueRaw === undefined ||
    stockValueRaw === ""
      ? null
      : toNumber(stockValueRaw, 0);
  const isStockVisible = Number(setting?.is_stock_visible ?? 1) === 1;
  ...
  const isOrderNowLocked = isBusy && !isOrderNowLoading;
  const isAddToCartLocked = isBusy && !isAddToCartLoading;
  ...
  return (
    <div className="theme-product-card overflow-hidden rounded-2xl border border-[#dde3ea] bg-white shadow-[0_2px_10px_rgba(15,23,42,0.08)]">
      ...
      <div className="px-3 pb-2">
        ...
        {isStockVisible && stockValue !== null ? (
          <div
        ...
        <div className="mt-1.5 flex items-end gap-2">
          ...
          {previousPrice > 0 ? (
            <span className="pb-0.5 text-sm font-semibold text-[#ef4444] line-through">
      ...
      <div className="space-y-2 px-2.5 pb-3">
        <button
          ...
          aria-disabled={isOutOfStock || isOrderNowLoading || isOrderNowLocked}
          className={`theme-btn-primary flex w-full items-center justify-center gap-2 rounded-xl border py-2.5 text-sm font-bold transition ${
            isOutOfStock
