...
const RelatedProductCard = ({ product }) => {
  ...
  return (
    <Link
      ...
      <h4 className="mt-3 line-clamp-2 text-sm font-semibold text-gray-800">
        {title}
      </h4>
...
const ProductDetailView = ({
  ...
  const videoSourceLabel = youtubeEmbedUrl
    ? "YouTube"
    : facebookEmbedUrl
      ? "Facebook"
      : "";
  ...
  useEffect(() => {
    ...
    if (activeImage > endIndex) {
      setThumbStartIndex(
        Math.min(activeImage - thumbsPerView + 1, maxThumbStartIndex),
      );
    }
    ...
    <div className="bg-[#edf1f7]">
      <div className="mx-auto max-w-[1120px] px-3 py-4 md:py-5">
          ...
          <div className="min-w-0 w-full max-w-full self-start rounded-2xl border border-[#d7dee9] bg-white p-3 shadow-sm md:p-4 lg:h-auto lg:w-[640px]">
            ...
            {productSku ? (
              <div className="mt-2 inline-flex max-w-full items-center gap-1.5 rounded-full border border-[#c7d9f2] bg-[#edf4ff] px-3 py-1 text-xs text-[#26436d]">
                ...
                <span className="truncate font-extrabold text-[#1f3558]">
                  {productSku}
                </span>
              </div>
            ) : null}
        ...
        <div className="mt-6 h-auto rounded-xl border border-[#c4cad6] bg-white p-4 shadow-sm md:p-6">
          ...
          <div className="product-detail-content prose h-auto max-w-none overflow-visible text-sm leading-7 text-gray-700">
            {renderTabContent()}
          </div>
        </div>
        ...
        {videoEmbedUrl ? (
          <div className="mt-6 rounded-xl border border-[#c4cad6] bg-white p-4 shadow-sm md:p-6">
            <h3 className="mb-3 text-lg font-semibold text-gray-900">
              Video {videoSourceLabel ? `(${videoSourceLabel})` : ""}
            </h3>
          ...
          <div className="mt-6 rounded-xl border border-[#c4cad6] bg-white p-4 shadow-sm md:p-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
              {relatedProducts.map((item) => (
                <RelatedProductCard
                  key={item.id || item.slug || item.href}
                  product={item}
                />
              ))}
            </div>
