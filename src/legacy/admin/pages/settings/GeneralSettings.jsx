...
const BUTTON_COLOR_ROWS = [
  ...
  },
  {
    ...
    fields: [
      {
        key: "button_secondary_bg_color",
        label: "Background",
        fallback: "#374151",
        placeholder: "#374151",
      },
      {
        key: "button_secondary_hover_color",
        label: "Hover",
        fallback: "#1f2937",
        placeholder: "#1f2937",
      },
  ...
  },
  {
    ...
    fields: [
      {
        key: "button_info_bg_color",
        label: "Background",
        fallback: "#0ea5e9",
        placeholder: "#0ea5e9",
      },
      {
        key: "button_info_hover_color",
        label: "Hover",
        fallback: "#0284c7",
        placeholder: "#0284c7",
      },
      {
        key: "button_info_text_color",
        label: "Text",
        fallback: "#ffffff",
        placeholder: "#ffffff",
      },
...
const GeneralSettings = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    is_stock_visible: 1,
    is_add_to_cart_show: 1,
    status: 1,
    logo: null,
    favicon: null,
  });
  const [fileErrors, setFileErrors] = useState({ logo: "", favicon: "" });
  const [uploadFileLists, setUploadFileLists] = useState({
    logo: [],
    favicon: [],
  });
  ...
  const handleEdit = (setting) => {
    ...
    setFormData({
      ...
      is_add_to_cart_show: Number(setting.is_add_to_cart_show ?? 1),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.append(
        "is_add_to_cart_show",
        formData.is_add_to_cart_show === 0 ? "0" : "1",
      );
      payload.append(
        "button_primary_color",
        formData.button_primary_color || "",
      );
      payload.append(
        "button_secondary_color",
        formData.button_secondary_color || "",
      );
      payload.append(
        "button_primary_text_color",
        formData.button_primary_text_color || "",
      );
      payload.append(
        "button_secondary_bg_color",
        formData.button_secondary_bg_color || "",
      );
      payload.append(
        "button_secondary_hover_color",
        formData.button_secondary_hover_color || "",
      );
      payload.append(
        "button_secondary_text_color",
        formData.button_secondary_text_color || "",
      );
      payload.append(
        "button_info_bg_color",
        formData.button_info_bg_color || "",
      );
      payload.append(
        "button_info_hover_color",
        formData.button_info_hover_color || "",
      );
      payload.append(
        "button_info_text_color",
        formData.button_info_text_color || "",
      );
      ...
      payload.append(
        ...
        String(Number(formData.footer_payment_enabled ?? 1)),
      );
      payload.append(
        "is_stock_visible",
        String(Number(formData.is_stock_visible ?? 1)),
      );
      ...
      if (isEditing && currentSetting) {
        ...
        if (hasSetting) {
          alert(
            "General setting already exists. Please edit the existing one.",
          );
          return;
        }
        await adminAction({
  ...
  const columns = [
    ...
    {
      header: "Add to Cart",
      accessor: "is_add_to_cart_show",
      width: "12%",
      render: (row) => (
        <span
          className={`px-3 py-1 rounded-full text-xs font-semibold ${
            Number(row.is_add_to_cart_show ?? 1) === 1
              ? "bg-green-100 text-green-800"
              : "bg-gray-200 text-gray-700"
          }`}>
          {Number(row.is_add_to_cart_show ?? 1) === 1 ? "Active" : "Inactive"}
        </span>
      ),
    },
    {
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      browser_tab_title: e.target.value,
                    })
                  }
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      footer_payment_enabled: Number(e.target.value),
                    })
                  }>
                  <option value={1}>Active</option>
                  <option value={0}>Inactive</option>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      is_stock_visible: Number(e.target.value),
                    })
                  }>
              ...
              <div>
                ...
                <select
                  className="w-full rounded-lg border border-gray-300 px-3 py-2"
                  value={Number(formData.is_add_to_cart_show ?? 1)}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      is_add_to_cart_show: Number(e.target.value),
                    })
                  }>
                  <option value={1}>Active</option>
                  <option value={0}>Inactive</option>
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-3">
                ...
                <Upload
                  ...
                  onChange={({ fileList }) =>
                    handleUploadChange("logo", fileList)
                  }
                  onRemove={() => {
                ...
                <p className="text-xs text-gray-500">
                  Label: 200 x 60 px (recommended) | Max size:{" "}
                  {MAX_IMAGE_SIZE_LABEL} | No width/height restriction.
                </p>
                {fileErrors.logo ? (
                  <p className="mt-1 text-xs text-red-600">{fileErrors.logo}</p>
                ) : null}
              </div>
              ...
              <div className="rounded-lg border border-dashed border-gray-300 bg-gray-50 p-3">
                ...
                <Upload
                  ...
                  onChange={({ fileList }) =>
                    handleUploadChange("favicon", fileList)
                  }
                  onRemove={() => {
                ...
                <p className="text-xs text-gray-500">
                  Label: 64 x 64 px (recommended) | Max size:{" "}
                  {MAX_IMAGE_SIZE_LABEL} | No width/height restriction.
                </p>
                {fileErrors.favicon ? (
