...
const OrderEdit = () => {
    ...
    useEffect(() => {
        ...
        setFormData((prev) => {
            ...
            const shippingArea = String(order?.shipping?.area || '').trim().toLowerCase();
            const shippingChargeAmount = Number(order?.shipping_charge ?? 0);
            const matched = shippingCharges.find((row) => (
                String(row?.name || '').trim().toLowerCase() === shippingArea
        ...
        try {
            await adminAction({
                ...
                body: {
                    ...
                    shipping_area: (
                        shippingCharges.find((row) => String(row.id) === String(formData.shipping_charge_id))?.name
