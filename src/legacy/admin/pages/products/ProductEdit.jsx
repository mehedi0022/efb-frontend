...
const ProductEdit = () => {
  ...
  useEffect(() => {
    if (productResponse?.success && productResponse?.data) {
      ...
      const sizeRows =
        Array.isArray(product?.size_pricing) && product.size_pricing.length > 0
  ...
  const handleYoutubeVideoChange = (value) => {
    setFormData((prev) => ({
      ...
      facebook_video_url: String(value || "").trim()
        ? ""
        : prev.facebook_video_url,
    }));
  };
  ...
  const handleFacebookVideoChange = (value) => {
    setFormData((prev) => ({
      ...
      youtube_video_url: String(value || "").trim()
        ? ""
        : prev.youtube_video_url,
    }));
  };
  ...
  const removeSizePricingRow = (rowIndex) => {
    setFormData((prev) => {
      const nextRows = prev.size_pricing.filter(
      ...
      if (!hasDefault) {
        ...
        const hasPrice =
          nextRows[0].price !== null &&
          nextRows[0].price !== undefined &&
          nextRows[0].price !== "";
        if (
  ...
  const updateSizePricingRow = (rowIndex, updates) => {
    setFormData((prev) => {
      ...
      const isUpdatingPrice = Object.prototype.hasOwnProperty.call(
        updates,
        "price",
      );
    ...
    setFormData((prev) => {
      ...
      const rows = prev.size_pricing.map((row) => {
        ...
        const shouldAutoSync =
          isPriceEmpty || row.auto_price_from_base === true;
        if (!shouldAutoSync) {
          return row;
        }
        ...
        if (
          Number(row.price) === Number(nextPrice) &&
          row.auto_price_from_base === true
        ) {
          return row;
        }
  ...
  const buildPayload = () => {
    ...
    payload.append(
      "youtube_video_url",
      String(formData.youtube_video_url || "").trim(),
    );
    payload.append(
      "facebook_video_url",
      String(formData.facebook_video_url || "").trim(),
    );
  ...
  const validateBeforeSubmit = () => {
    ...
    if (
      ...
      formData.purchase_price === undefined
    ) {
    ...
    const hasYoutube =
      String(formData.youtube_video_url || "").trim().length > 0;
    const hasFacebook =
      String(formData.facebook_video_url || "").trim().length > 0;
    if (hasYoutube && hasFacebook) {
      return "You can set either YouTube or Facebook video, not both.";
    }

    const validRows = (formData.size_pricing || []).filter(
      (row) => row.size_id || row.price !== null,
    );
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-[2fr_1fr_160px_48px] md:items-end">
                    <div>
                      ...
                      <Select
                        ...
                        value={row.size_id}
                        onChange={(value) =>
                          updateSizePricingRow(rowIndex, { size_id: value })
                        }
                    ...
                    <div>
                      ...
                      <InputNumber
                        ...
                        value={row.price}
                        onChange={(value) =>
                          updateSizePricingRow(rowIndex, { price: value })
                        }
                        min={0}
                        className="!w-full"
                        placeholder="0.00"
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Recommended size: 1200 x 1200 px (square)
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Recommended size: 1200 x 1200 px (consistent ratio)
                </p>
              </div>
            </div>
          </Card>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    handleYoutubeVideoChange(event.target.value)
                  }
                  placeholder="https://www.youtube.com/watch?v=... or YouTube video ID"
                  style={{ width: "100%" }}
                />
                <p className="mt-1 text-xs text-gray-500">
                  Use either YouTube or Facebook video.
                </p>
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(event) =>
                    handleFacebookVideoChange(event.target.value)
                  }
                  placeholder="https://www.facebook.com/.../videos/..."
                  style={{ width: "100%" }}
                />
                <p className="mt-1 text-xs text-gray-500">
                  Provide only one video source.
                </p>
              </div>
